...
namespace UCS.Logic
{
    class ClientAvatar : Avatar
    {
        ...
        public ClientAvatar() : base()
        {
            ...
            // qui è da aggiungere il coso delle leghe sennò non rimane statico ma solo quando l'utente logga
        ...
        public ClientAvatar(long id) : this()
        {
            ...
            int randomtrophies = rnd.Next(400, 3500);
            ...
            m_vScore = randomtrophies;
            // 8 = gold 2; || 7 = gold 3; || 9 = gold 1; || 6 = argento 1; || 1 = bronzo 3 || 2 = bronzo 2 || 12 = cristallo 1 || 15 = MASTER || 18 CHAMP | 21 TITAN //
            //400 BRONZO || 800 ARGENTO || ORO 1400 | CRISTALLO 2000 | MASTER 2600 | CHAMP 3200 | TITAN 4100
            if (m_vScore >= 400 && m_vScore <= 800)
            {
                m_vLeagueId = 3;
            }
            else if (m_vScore >= 801 && m_vScore <= 1400)
            {
                m_vLeagueId = 6;
            }
            else if (m_vScore >= 1401 && m_vScore <= 2000)
            {
                m_vLeagueId = 9;
            }
            else if (m_vScore >= 2001 && m_vScore <= 2600)
            {
                m_vLeagueId = 12;
            }
            else if (m_vScore >= 2601 && m_vScore <= 3200)
            {
                m_vLeagueId = 15;
            }
            else if (m_vScore >= 3201 && m_vScore <= 4099)
            {
                m_vLeagueId = 18;
            }
            else if (m_vScore >= 4100)
            {
                m_vLeagueId = 21;
            } 
           // m_vLeagueId = 9;
           ...
           // sembra non funzionare questo | SetResourceCount(ObjectManager.DataTables.GetResourceByName("Diamonds"), Convert.ToInt32(ConfigurationManager.AppSettings["startingGems"]));
        }
